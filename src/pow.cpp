...
{
    /* current difficulty formula, cashhand - DarkGravity v3, written by Evan Duffield - evan@dashpay.io */
    ...
    if (BlockLastSolved->nHeight <= 21000) { // reset diff
        return consensus.powLimit.GetCompact();
    }

    if (pindexLast->nHeight > consensus.height_last_PoW ) {
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader* pblock)
{
	...
	if (BlockLast->nHeight <= 21000){ // DarkGravityWave PoW switch
