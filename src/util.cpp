...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL) {
        ...
        if (ptrCategory.get() == NULL) {
            ...
            // "cashhand" is a composite category enabling all CASHHAND-related debug output
            if (ptrCategory->count(std::string("cashhand"))) {
...
// Windows < Vista: C:\Documents and Settings\Username\Application Data\cashhanddata
// Windows >= Vista: C:\Users\Username\AppData\Roaming\cashhanddata
// Mac: ~/Library/Application Support/cashhanddata
// Unix: ~/.cashhanddata
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "cashhanddata";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "cashhand.conf"));
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty cashhand.conf if it does not exist
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override cashhand.conf
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "cashhandd.pid"));
