...
MasterNodesWidget::MasterNodesWidget(CASHHANDGUI *parent) :
    PWidget(parent),
    ...
    isLoading(false)
{
    ...
    //ui->btnAboutController->setVisible(false);
}
    ...
    if (dialog->exportMN) {
        if (ask(tr("Remote Masternode Data"),
                tr("You are just about to export the required data to run a Masternode\non a remote server to your clipboard.\n\n\n"
                   "You will only have to paste the data in the cashhand.conf file\nof your remote server and start it, "
    ...
    if (boost::filesystem::exists(pathBootstrap)) {
        ...
        if (lineCopy.size() == 0) {
            lineCopy = "# Masternode config file\n"
                                    ...
                                    "# Example: mn1 127.0.0.2:17520 93HaYBVUCYjEMeeH1Y4sBGLALQZE1Yc1K64xiqgX37tGBDQL8Xg 2bcd3c84c84f87eaa86e4e56834c92927a07f9e18718810b92e0d0324456a67c 0\n";
        }
...
void MasterNodesWidget::onCreateMNClicked(){
    ...
    if (walletModel->getBalance() <= (COIN * GetMstrNodCollateral(chainActive.Height()))) {
        inform(tr("Not enough balance to create a masternode, %1 CHND with fee required.").arg(GetMstrNodCollateral(chainActive.Height())));
        return;
    }
    showHideOp(true);
