...
#if defined(NDEBUG)
#error "CASHHAND cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    // LogPrintf("Money Supply = %f AND Max Moneyout = %f AND Subsidy = %f\n", nMoneySupply, Params().GetConsensus().nMaxMoneyOut, nSubsidy);
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from CASHHAND
...
{
    util::ThreadRename("cashhand-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // Track zCHND money supply in the block index
    if (!UpdateZPIVSupply(block, pindex))
        return state.DoS(100, error("%s: Failed to calculate new zCHND supply for block=%s height=%d", __func__,
...
bool static LoadBlockIndexDB(std::string& strError)
{
    ...
    for (const PAIRTYPE(int, CBlockIndex*) & item : vSortedByHeight) {
        ...
        if (pindex->IsValid(BLOCK_VALID_TREE) && (pindexBestHeader == NULL || CBlockIndexWorkComparator()(pindexBestHeader, pindex))) {
            ...
            } else if (!fBlockDatabaseForkFound && pindex->nHeight > 925000) { // If higher than the last auto-checkpoint, check every block (Not the most optimized)
                if (!Checkpoints::CheckBlock(pindex->nHeight, pindex->GetBlockHash())) {
                    fBlockDatabaseForkFound = true;
                    LogPrintf("LoadBlockIndexDB() : (>925000) Possible fork detected, but we have no more checkpoints to compare against, assuming irreversibly forked...\n");
                }
...
std::string GetWarnings(std::string strFor)
{
    ...
    if (fLargeWorkForkFound) {
        ...
        strStatusBar = strRPC = _("Warning: Your blockchain doesn't match the required checkpoints! You have forked, please resync CASHHAND Core.");
    }
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // CASHHAND: We use certain sporks during IBD, so check to see if they are
	...
	if(chainActive.Tip()->nHeight > 20000){
        version_old = "< 2.3.0.0";
        if (pfrom->cleanSubVer == "/Cashhand Core:2.2.1/" || pfrom->cleanSubVer == "/Cashhand Core:2.1.0.0/" || pfrom->cleanSubVer == "/Cashhand Core:2.0.0/" || pfrom->cleanSubVer == "/Cashhand Core:1.1.1.1/" || pfrom->cleanSubVer == "/Cashhand Core:1.0.1.1/" || pfrom->cleanSubVer == "/Cashhand Core:1.0.0/") {
        ...
        LogPrintf("timeOffset: %d seconds - peer: %s\n",  nTimeOffset, pfrom->addr.ToString().c_str());
    }
        ...
        if (!tx.HasZerocoinSpendInputs() && AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs, false, ignoreFees)) {
            ...
            // Zerocoin is permanently disabled, simply reject all incoming Zerocoin spends
            state.DoS(10, error("%s : Zerocoin transactions have been permanently disabled",
                __func__), REJECT_INVALID, "bad-tx");
