...
*Setup instructions for a gitian build of Cashhand Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Cashhand
...
to the Cashhand GitHub release page.
...
- [Building Cashhand Core](#building-cashhand-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/cashhand-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/cashhand-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/cashhand-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/cashhand-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/cashhand-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/cashhand-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/CashHand-Project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@cashhand.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/cashhand-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/cashhand-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/cashhand-osx-*-build.assert
```
