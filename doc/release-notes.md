...
Cashhand Core version *version* is now available from:  <https://github.com/CashHand-Project/cashhand/releases>
...
Enabling major improvements over the keystore management, the Cashhand wallet doesn't require regular backups as before, keys are following a deterministic creation path that can be verified at any time (before HD Wallet, every keypair was randomly created and added to the keypool, forcing the user to backup the wallet every certain amount of time or could end up loosing coins forever if the latest `wallet.dat` was not being used).
...
An extended description of this large work can be found in the PR [here](https://github.com/CashHand-Project/cashhand/pull/1327).
...
Automatic zCHND backup has been disabled. Thus, the following configuration options have been removed  (either as entries in the cashhand.conf file or as startup flags):
- `autozchndbackup`
- `backupzchnd`
- `zchndbackuppath`
...
The default value for the stake-split threshold has been lowered from 2000 CHND, down  to 500 CHND.


Dependencies
------------

The minimum required version of QT has been increased from 5.0 to 5.5.1 (the [depends system](https://github.com/CashHand-Project/cashhand/blob/master/depends/README.md) provides 5.9.7)
...
- The output of `getstakingstatus` was reworked. It now shows the following information:
  ```
  {
     ...
     "staking_enabled": true|false,      (boolean) whether staking is enabled/disabled in cashhand.conf
     "coldstaking_enabled": true|false,  (boolean) whether cold-staking is enabled/disabled in cashhand.conf
     "haveconnections": true|false,      (boolean) whether network connections are present
     "mnsync": true|false,               (boolean) whether masternode data is synced
     "walletunlocked": true|false,       (boolean) whether the wallet is unlocked
     "stakeablecoins": n,                (numeric) number of stakeable UTXOs
     "stakingbalance": d,                (numeric) CHND value of the stakeable coins (minus reserve balance, if any)
...
The p2p alert system has been removed in [#1372](https://github.com/CashHand-Project/cashhand/pull/1372) and the 'alert' message is no longer supported.

### GUI

Keyboard navigation: dialogs can now be accepted with the `ENTER` (`RETURN`) key, and dismissed with the `ESC` key ([#1392](https://github.com/CashHand-Project/cashhand/pull/1392)).

### RPC/REST

### Wallet

The `-reservebalance` configuration/startup option has been removed ([PR #1373](https://github.com/CashHand-Project/cashhand/pull/1373)).
