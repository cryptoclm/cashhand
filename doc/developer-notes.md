...
- [Development guidelines](#development-guidelines)
    - [General Cashhand Core](#general-cashhand-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Cashhand Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play CHNDs (tCHND)" on the test network, if you
...
Cashhand Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/CashHand-Project/cashhand/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_cashhand
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_cashhand --log_level=test_suite
$ valgrind -v --leak-check=full src/cashhandd -printtoconsole
```
...
# A coverage report will now be accessible at `./test_cashhand.coverage/index.html`.
```
...
- BitcoinMiner : Generates CHNDs (if wallet is enabled).
...
However, in open source software such as Cashhand Core, where everyone uses
...
pay attention to for reviewers of Cashhand Core code.

General Cashhand Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: Cashhand Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against Cashhand Core so that they can be integrated
...
file descriptors it will cause problems with Cashhand's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:CashHand-Project/cashhand.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `cashhand-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `cashhand-cli` and the GUI debug console use this table to determine how to
